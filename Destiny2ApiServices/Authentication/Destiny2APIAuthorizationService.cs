namespace DestinyLoadoutBuilder.Services.Authentication
{
    ...
    public class Destiny2APIAuthorizationService : IDestiny2APIAuthorizationService
    {
        ...
        /// <returns></returns>
        public async Task GetOauthTokenAsync(string uri, Guid generatedStateValue)
        {
            //TODO: explore whether a brand new HttpClient instance is necessary
            using HttpClient client = new HttpClient();
            string authCode = GetAuthCodeFromURI(uri, generatedStateValue);
        ...
        /// <summary>
        /// Extracts the Authorization code from the current URI if the state value echoed from the server
        /// matches the original state value generated by this application.
        /// </summary>
        /// <param name="uri">The current URI</param>
        /// <returns></returns>
        private string GetAuthCodeFromURI(string uri, Guid generatedStateValue)
        {
            Uri theUri = new Uri(uri);
            string authCode = HttpUtility.ParseQueryString(theUri.Query).Get(0);
            string stateValueFromServer = GetStateValueFromURI(uri);
            return StateValueIsValid(stateValueFromServer, generatedStateValue) ? authCode : throw new InvalidOperationException("The state value did not match the server");
        }
        ...
        /// <returns></returns>
        private bool StateValueIsValid(string state, Guid generatedStateValue)
        {
            if (Guid.Parse(state).Equals(generatedStateValue))
            {
                return true;
            }
            return false;
    ...
    public interface IDestiny2APIAuthorizationService
    {
        ...
        Task GetOauthTokenAsync(string uri, Guid generatedStateValue);
    }
}